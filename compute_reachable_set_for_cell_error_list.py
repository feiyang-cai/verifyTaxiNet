...
def main():
    ...
    result_folder = "./reachable_sets_error_list"
    ...
    with open(error_list_file, 'r') as file:
        for line in file:
            line = line.strip()
            tuple_data = eval(line)
            p_lb, p_ub, theta_lb, theta_ub = tuple_data
            ...
            if not reachable_cells == {(-2, -2, -2, -2)}:
                for split_tolerance in [1e-2, 1e-1]:
            ...
            print("--------------------------------------------------------")
            reachable_set_file = os.path.join(result_folder, f"reachable_sets_{p_lb}_{p_ub}_{theta_lb}_{theta_ub}.pkl")
            with open(reachable_set_file, "wb") as f:
                pickle.dump(reachable_cells, f)
